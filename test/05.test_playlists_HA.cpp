  ...
  if (estadoActual == DIAL_TONE || estadoActual == COUNTING) {
    ...
    if (estadoActual == COUNTING) {
      if (millis() - tiempoInicioConteo > TIMEOUT_PULSOS) {
        ...
        // Si es 1 o 2, llamar webhook y salir
        if (contadorPulsos == 1 || contadorPulsos == 2) {
          HTTPClient http;
          String webhookUrl = "http://10.0.1.3:8123/api/webhook/ericofono-lista0" + String(contadorPulsos) + "-despacho";
        ...
        // Para números 3-9: Construir URL de Navidrome
