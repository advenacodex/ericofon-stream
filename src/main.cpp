...
void loop() {
  ...
  if (ganchoActual != ganchoAnterior) {
    if (millis() - ultimoDebounceGancho > DEBOUNCE_GANCHO) {
      ...
      if (ganchoAnterior == HIGH && ganchoActual == LOW) {
        ...
        audio.connecttoFS(LittleFS, "/system_ready.mp3");
        Serial.println("Reproduciendo: System Ready");
      }
  ...
  if (estadoActual == DIAL_TONE || estadoActual == COUNTING) {
    ...
    // Timeout: terminar conteo y ejecutar acción
    if (estadoActual == COUNTING) {
      if (millis() - tiempoInicioConteo > TIMEOUT_PULSOS) {
        Serial.print("Número marcado: ");
        Serial.println(contadorPulsos);

        // Números 1-9: llamar webhook de Music Assistant
        if (contadorPulsos >= 1 && contadorPulsos <= 9) {
          HTTPClient http;
          String webhookUrl =
              "http://10.0.1.3:8123/api/webhook/ericofono-lista0" +
              String(contadorPulsos) + "-despacho";
          ...
          // Volver a IDLE sin reproducir nada
          estadoActual = IDLE;
          audio.stopSong();
          contadorPulsos = 0;
          Serial.println("Webhook ejecutado, sistema en reposo");
          return;
        }
...
// Esta función es un STREAM local, para otras funcionalidades
String construirURLNavidrome(int numero) {
  ...
  streamUrl += "&format=mp3";
  streamUrl += "&maxBitRate=128";
